...
clust_PSC_n <- str_which(input0, "^clust_PSC=")
clust_PSC <- str_remove(input0[clust_PSC_n], "^clust_PSC=")
rm(clust_PSC_n)

prob1_PSC_c <- str_which(input0, "^prob1_PSC=")
prob1_PSC <- str_remove(input0[prob1_PSC_c], "^prob1_PSC=") %>%
  as.integer()
rm(prob1_PSC_c)

prob2_PSC_c <- str_which(input0, "^prob2_PSC=")
prob2_PSC <- str_remove(input0[prob2_PSC_c], "^prob2_PSC=") %>%
  as.integer()
rm(prob2_PSC_c)

cov1_PSC_c <- str_which(input0, "^cov1_PSC=")
cov1_PSC <- str_remove(input0[cov1_PSC_c], "^cov1_PSC=") %>%
  as.integer()
rm(cov1_PSC_c)

cov2_PSC_c <- str_which(input0, "^cov2_PSC=")
cov2_PSC <- str_remove(input0[cov2_PSC_c], "^cov2_PSC=") %>%
  as.integer()
rm(cov2_PSC_c)

alig_PSC_c <- str_which(input0, "^alig_PSC=")
alig_PSC <- str_remove(input0[alig_PSC_c], "^alig_PSC=") %>%
  as.integer()
rm(alig_PSC_c)
...
## load protDF
protDF_p <- paste0(input, "/02/02_04_genome_protDF_PCs.RDS")
rm(input)
prot_DF <- readRDS(protDF_p) %>%
  filter(PC_ID != "") %>%
  mutate(PC_ID = as.numeric(PC_ID))
rm(protDF_p)


### 01. Write Clusters into mfasta files ########################################
st01_D <- paste0(P_D, "/03_01_PC_mfasta")
if(dir.exists(st01_D)){unlink(st01_D, recursive = TRUE)}
dir.create(st01_D)

prot_DF_PC_clust <- prot_DF %>%
  group_by(PC_ID) %>%
  nest()
rm(prot_DF)
# sprintf variable
d <- nrow(prot_DF_PC_clust) %>%
  as.character() %>%
  nchar()
d <- paste0("%0", d, "d")

prot_DF_PC_clust <- prot_DF_PC_clust %>%
  mutate(clust_ID = sprintf(d, PC_ID)) %>%                                     ###make PC_ID = clust_ID
...
write.fasta_clustDf_fun <- function(DF, path, clust_ID)
{
  ...
  DF <- DF %>%
    mutate(PC_prot_ID = paste0("cluster_", clust_ID, "_", protein_ID))
  
  name_ls <- DF$PC_prot_ID %>%
    as.list()
  
  write.fasta(sequences = seq_ls, names = name_ls, file.out = path, as.string = TRUE)
}

todel <- mapply(write.fasta_clustDf_fun, prot_DF_PC_clust$data, prot_DF_PC_clust$mfasta_p, prot_DF_PC_clust$clust_ID)

rm(d, todel)


### 02. Align with Clustal-Omega ######################################################
st02_D <-paste0(P_D, "/03_02_aligned_PCs")
...
prot_DF_PC_clust$no_prots <- lapply(X = prot_DF_PC_clust$data, FUN = nrow) %>% unlist()
malig_DF <- prot_DF_PC_clust %>%
  filter(no_prots > 1) %>%
...
if(length(malig_st) > 1 | (length(malig_st) == 1 & malig_st != 0))
{
  ...
  saveRDS(malig_report_DF, file = paste0(P_D, "/error_DF_03b_02.RDS"))
  
  print("Incomplete multiple alignment creation in step 3B_02. From ", nrow(malig_DF), " multiple alignments, ", nrow(malig_report_DF), " were NOT created. Aborting VirClust.")
...
st03_D <- paste0(P_D, "/03_03_aligPCs_a3m")
...
prot_DF_PC_clust <- prot_DF_PC_clust %>%
  ...
  mutate(reformat_cmd = paste0(com, alig_mfasta_p, " ", a3m_p,
                               " > /dev/null 2> /dev/null"))
print(prot_DF_PC_clust$reformat_cmd[1])

#plan(multisession, workers = cpu)
todel <- map(.x = prot_DF_PC_clust$reformat_cmd, .f = system)
#plan(sequential)
...
st04_D <- paste0(P_D, "/03_04_PCs_hmms")
...
for(i in 1:nrow(prot_DF_PC_clust))
{
  prot_DF_PC_clust[[i, "hhmake_status"]] <- sys::exec_wait(cmd = "hhmake", 
                                                           args = c("-i", prot_DF_PC_clust$a3m_p[[i]],
                                                                    "-o", prot_DF_PC_clust$hmm_p[[i]],
                                                                    "-name", prot_DF_PC_clust$hmm_name[[i]],
                                                                    "-id", "100",
                                                                    "-diff", "1000000"), 
                                                           std_out = prot_DF_PC_clust$hhmake_stdout[[i]],
                                                           std_err = prot_DF_PC_clust$hhmake_stderr[[i]])
}
rm(i)
...
hhmake_st <- unique(prot_DF_PC_clust$hhmake_status)
if(length(hhmake_st) > 1 | (length(hhmake_st) == 1 & hhmake_st != 0))
{
  hhmake_report_DF <- prot_DF_PC_clust %>%
    filter(hhmake_status > 0)
  
  saveRDS(hhmake_report_DF, file = paste0(P_D, "/error_DF_03b_04.RDS"))
  
  print(paste0("Incomplete HMM creation in step 3B_04. From ", nrow(prot_DF_PC_clust), " HMM profiles, ", nrow(hhmake_report_DF), " were NOT created. Aborting VirClust."))
...
check_exec_st_fun(st = cstranslate_st, who = "cstranslate")
...
st06_D <- paste0(P_D, "/03_06_hhsearch_out")
...
#                              " -d ", st05_D, "/search_DB -id 100 -diff 0 -cpu 20 -p 50 -z 1 -Z ", as.character(nrow(prot_DF_PC_clust) + 600000),
...
for(i in 1:nrow(prot_DF_PC_clust))
{
  prot_DF_PC_clust[[i, "hmmsearch_status"]] <- sys::exec_wait(cmd = "hhsearch", 
                                                              args = c("-i", prot_DF_PC_clust$hmm_p[[i]],
                                                                       "-o", prot_DF_PC_clust$hhr_p[[i]],
                                                                       "-blasttab", prot_DF_PC_clust$hhsearch_tsv_p[[i]],
                                                                       ...
                                                                       "-p", "50",
                                                                       "-z", "1", 
                                                                       "-Z", as.character(nrow(prot_DF_PC_clust) + 600000)), 
                                                              std_out = prot_DF_PC_clust$hhsearch_log_p[[i]],
                                                              std_err = prot_DF_PC_clust$hhsearch_err_p[[i]])
}
rm(i)


### 07. Filter and write 2nd abc file ##########################################
st07_D <- paste0(P_D, "/03_07_in_mcl")
if(file.exists(st07_D)){unlink(st07_D, recursive = TRUE)}
dir.create(st07_D)

### #test
#tsv_p <- prot_DF_PC_clust$hhsearch_log_p[1]
#clust_ID <- prot_DF_PC_clust$clust_ID[1]
...
import_hhsearch_res_fun <- function(tsv_p, clust_ID)
{
  if(file.size(tsv_p) > 0)
  {
    ...
    score_TB <- score_TB %>%
      ...
      filter((Prob >= prob1_PSC & scov >= cov1_PSC & qcov >= cov1_PSC) | (Prob >= prob2_PSC & scov >= cov2_PSC & qcov >=cov2_PSC & Cols >= alig_PSC)) %>% #I'm filtering hits
...
score_TB_ls <- map2(.x = prot_DF_PC_clust$hhsearch_log_p, .y = prot_DF_PC_clust$clust_ID, .f =  import_hhsearch_res_fun)
...
if(clust_PSC == "bitscore")
{
  for_mcl_TB <- score_TB %>%
    select(query_cluster, target_cluster, Score)
}

if(clust_PSC == "norm_bitscore")
{
...
### 08. Making PSCs #######################################################
st08_D <- paste0(P_D, "/03_08_mcl_out")
if(dir.exists(st08_D)){unlink(st08_D, recursive = TRUE)}
dir.create(st08_D)

mcl_clust_path <- paste0(st08_D, "/mcl_super_clusters.txt")
...
check_exec_st_fun(st=mcl_status, who = "MCL for PSCs")
rm(hmm_abc_file, mcl_status)


### 17. Add PSCs to prot_DF_PC_clust
...
for(i in 1:nrow(mcl_super_clust_DF))
{
  ...
  prot_DF_PC_clust[prot_DF_PC_clust$clust_ID %in% prot_ls, "PSC_ID"] <- i
  rm(prot_ls)
}
...
prot_DF_PC_clust <- prot_DF_PC_clust %>%
  ...
  unnest(data) %>%
  ungroup() %>%
  select(genome_name, gene_ID, gene_start, gene_end, gene_length, strand, frame, protein_name, protein_ID, PC_ID, PSC_ID, protein_length, protein_seq)

rm(st01_D, st02_D, st03_D, st04_D, st05_D, st06_D, st07_D, st08_D)


### 18. Save prot_DF_PC_clust #############################################################
DF_path <- paste0(P_D, "/03_09_genome_protDF_PCs_PSCs.RDS")
if(file.exists(DF_path)){file.remove(DF_path)}
saveRDS(prot_DF_PC_clust, DF_path)
rm(DF_path)


tsv_p <- paste0(P_D, "/03_09_genome_protDF_PCs_PSCs.tsv")
write.table(x = prot_DF_PC_clust, file = tsv_p, sep = "\t", row.names = FALSE, col.names = TRUE)
rm(tsv_p)


print("Step 3, from PCs to PSCs, has finished successfully.")